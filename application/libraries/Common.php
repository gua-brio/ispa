<?php 
 class Common{
	...
	static function getPass($pass = false){
	    return md5(sha1($pass));
	}
	public function echo_password(){
		echo self::getPass("brio@@2016");
	}
  ...
  static function appList($appointment = false,$mode = "fixed"){
	...
	static function busServices($bus = false){
		if ($bus) {			
			...
			return isset($sl) ? $sl :"We have no services listed at the moment.";
		}
		return false;
	}
	...
	static function renderAppointment($appointment = false){
		if ($appointment) {
			...
			foreach ($services as $serv) {
				...
				if(sizeof($services) == 1){
					$ss .= $s["name"];
				}elseif($serv["service_id"] == $services[sizeof($services) -1]["service_id"]){
					$ss .= $s["name"].", ";
				}
			}			
			return '
	...
	static function refund($app = false){
		$CI = &get_instance();
		if ($app) {
